...
export default function SettingsPopup({id, joinedRoomId, setModsData}: SettingsPopupProps) {
  const [ showUnModConfirmation, setShowUnModConfirmation ] = useState<boolean>(false)

  function cancel(e: MouseEvent<HTMLSpanElement>) {
    e.stopPropagation()
  ...
  return (
    <div className={`settings-${id} absolute top-[40%] right-2 z-10 bg-myBackground text-myText px-2 py-1 rounded-md transition-all duration-150`}
      style={{visibility: 'hidden'}}
    >
      <div className='bg-mySecondary px-2 py-[2px] rounded-md my-[0.1rem] hover:bg-red-700 hover:scale-x-105 active:scale-x-100 transition-all duration-100'
